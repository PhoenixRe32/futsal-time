...
function PasswordWidget(divid,pwdname)
{
	...
	if ( currentLang == 'en' ) {	
		...
		this.txtShow = 'Εμφάνισε';
		this.txtMask = 'Κρύψε';
		this.txtGenerate = 'Δημιούργησε';
		this.txtWeak='αδύναμος';
		this.txtMedium='μέτριος';
		this.txtGood='καλός';
	}
	else {	
		this.txtShow = 'Show';
...
function _MakePWDWidget()
{
	...
	code += "<input type='password' class='pwdfield' name='"+pwdname+"' id='"+this.pwdfieldid+"' tabindex='540'>";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' class='pwdfield' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;' tabindex='541'>";
