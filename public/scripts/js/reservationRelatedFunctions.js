...
function reservation(size, arenaName, arenaID, dateTime, requestType, challenges, contacts, element, smsSendingMan, smsSendingCus) 
{
	...
	if ( currentLang == null )
		currentLang = 'en';
	var phonesArr = contacts.split(" ");
	var msg="";
	
	if ( currentLang == 'en' ) 
		msg = "Online bookings disabled for now.<br><br>For bookings please call at one of the folowing numbers:<br><br>";
	else if ( currentLang == 'gr' ) 
		msg = "Δυστυχώς δεν μπορείτε να κλείσετε μέσω του διαδικτύου τη δεδομένη στιγμή. <br><br> Για κρατήσεις τηλεφωνήστε στο:<br><br>";
	
	for (var i = 0; i < phonesArr.length; i+=2) {
		msg += "<a href=\"tel:"+phonesArr[i]+"\">"+phonesArr[i]+"</a> | ";
	}
	msg +="<br>";
	message_box.show_message('<u>Futsal-Time</u>',msg, true); return;
