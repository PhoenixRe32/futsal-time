<?php 
	...
	/* function sendEmail($arenaName, $date, $time, $fieldSize, $type, $desc, $email)
	{
		...
		$titles = array();
		$titles['MATCH'] = "Futsal-Time [{$arenaName}] : Booking Details for {$date}";
		$titles['CANCEL_M'] = "Futsal-Time [{$arenaName}] : Booking Cancellation for {$date}";
		$titles['CANCEL_C'] = "Futsal-Time [{$arenaName}] : Challenge Cancellation for {$date}";
		$titles['CANCEL_D'] = "Futsal-Time [{$arenaName}] : Challenge Cancellation for {$date}";
		$titles['CHALLENGE_IS'] = "Futsal-Time [{$arenaName}] : Booking Details for {$date}";
		$titles['CHALLENGE_AC'] = "Futsal-Time [{$arenaName}] : An opponent has been found for {$date}";

		
		$from = array();
		$from['MATCH'] = "bookings@futsal-time.com";
		$from['MATCH_C'] = "bookings@futsal-time.com";
		$from['CHALLENGE'] = "bookings@futsal-time.com";
		$from['CANCEL'] = "bookings@futsal-time.com";
		
		// Create the message
		$message = Swift_Message::newInstance()
		// Give the message a subject
		->setSubject($titles[$desc])
		// Set the From address with an associative array
		->setFrom(array($from[$type] => $arenaName))
	...
	{	
		...
		*/
		...
		$response = $ws->submitSM('123456789', $recipients, $sms, "GSM");
